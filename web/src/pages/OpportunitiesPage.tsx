  ...
  return (
    <div className="p-6 md:p-8 max-w-3xl mx-auto pb-32">
      <div className="flex items-start gap-3 mb-8">
        <Lightbulb className="w-8 h-8 text-[var(--text-primary)] mt-1" />
      ...
      <div className="bg-[var(--bg-surface)] border border-[var(--border-hairline)] rounded-xl p-4 mb-8">
        ...
        >
          <input
            required
            placeholder="Title (e.g. Register for conference)"
            value={title}
          ...
          <button
            type="submit"
            className="md:col-span-2 w-full bg-[var(--text-primary)] text-[var(--bg-base)] py-2 rounded text-sm font-medium hover:opacity-90 flex items-center justify-center gap-2 mt-2"
          >
      ...
      <div className="space-y-3">
        {deadlines.length === 0 ? (
          <p className="text-sm text-[var(--text-secondary)] text-center py-8">
            No deadlines currently tracked.
          </p>
        ) : (
          deadlines.map((d) => {
            const isPast =
            ...
            return (
              <div
                key={d.id}
                className={`p-4 border border-[var(--border-hairline)] rounded-lg flex items-center justify-between gap-4 bg-[var(--bg-surface)] ${isPast ? "opacity-50" : ""}`}
              >
