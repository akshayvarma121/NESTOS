    ...
    <div className="p-6 md:p-8 max-w-4xl mx-auto pb-32">
      <div className="flex justify-between items-start mb-8">
        <div>
          ...
          <p className="text-[var(--text-secondary)] text-sm">Link up with friends using invite codes.</p>
        </div>
        <button onClick={handleLogout} className="text-xs bg-[var(--bg-surface-raised)] hover:bg-[var(--border-hairline)] px-3 py-1.5 rounded transition-colors">
          Sign out
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div className="bg-[var(--bg-surface)] border border-[var(--border-hairline)] rounded-xl p-6">
          <h2 className="text-lg font-medium mb-4 flex items-center gap-2">
            <UserPlus className="w-4 h-4 text-[var(--text-secondary)]" /> My Invite Code
          </h2>
          
          {profile ? (
            <div className="space-y-4">
              <p className="text-sm text-[var(--text-secondary)]">Share this code with your partner so they can link to your profile.</p>
              <div className="flex items-center justify-center p-6 bg-[var(--bg-base)] border border-[var(--border-hairline)] rounded-lg">
                <span className="text-4xl font-mono tracking-[0.5em] text-[var(--text-primary)]">{profile.invite_code}</span>
              </div>
        ...
        <div className="bg-[var(--bg-surface)] border border-[var(--border-hairline)] rounded-xl p-6">
          <h2 className="text-lg font-medium mb-4 flex items-center gap-2">
            <LinkIcon className="w-4 h-4 text-[var(--text-secondary)]" /> Link a Partner
          </h2>
          
          <form onSubmit={handleConnect} className="space-y-4">
            <p className="text-sm text-[var(--text-secondary)]">Enter a friend's 6-character invite code to connect with them.</p>
            <div className="flex gap-2">
              ...
              />
              <button 
                type="submit" disabled={connecting || inviteCode.length < 6}
      ...
      <div className="mt-12">
        <div className="flex justify-between items-center mb-6">
          ...
          <button onClick={fetchData} className="p-2 text-[var(--text-secondary)] hover:text-[var(--text-primary)] transition-colors rounded-full hover:bg-[var(--bg-surface-raised)]">
        ...
        {partners.length === 0 ? (
          ...
          <div className="grid grid-cols-1 gap-4">
            {partners.map(p => (
              <div key={p.id} className="bg-[var(--bg-surface)] border border-[var(--border-hairline)] rounded-xl p-6">
                  ...
                  {p.macroProgress.map((m: any, idx: number) => (
                    <div key={idx}>
                      <div className="flex justify-between text-xs mb-1">
                        <span className="text-[var(--text-secondary)]">{m.title}</span>
                        <span>{m.percent}%</span>
                      </div>
                      <div className="h-1.5 w-full bg-[var(--bg-base)] rounded-full overflow-hidden border border-[var(--border-hairline)]">
                        <div className="h-full bg-[var(--accent)] transition-all duration-500" style={{ width: `${Math.max(0, Math.min(100, m.percent))}%` }} />
                      </div>
                    </div>
                  ))}
                  {p.macroProgress.length === 0 && <p className="text-xs text-[var(--text-tertiary)] italic">No active macro goals.</p>}
                </div>
              </div>
            ))}
          </div>
