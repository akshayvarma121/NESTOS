...
export default function VaultPage() {
  ...
  const revealAndCopy = async (id: string) => {
    try {
      ...
      window.dispatchEvent(
        new CustomEvent("vault_copy", { detail: "Copied — clears in 30s" }),
      );
        ...
        <div className="w-full max-w-sm px-8">
          <div className="text-center mb-8">
            ...
            <h1 className="text-2xl font-semibold mb-2">
              {isSetup ? "Unlock Vault" : "Setup Vault"}
            </h1>
            <p className="text-[var(--text-secondary)] text-sm">
              {isSetup
                ? "Enter your PIN"
                : "Create a master PIN. This cannot be recovered if lost."}
            </p>
          </div>
  ...
  return (
    <div className="p-6 md:p-8 max-w-4xl mx-auto h-[calc(100vh-80px)] lg:h-screen flex flex-col relative">
          ...
          <div className="flex items-center gap-3 mb-8">
            ...
            <div className="relative group cursor-help ml-2 mt-1">
              ...
              <div className="absolute left-0 top-full mt-2 w-64 p-3 bg-[var(--bg-surface-raised)] border border-[var(--border-hairline)] rounded-lg shadow-xl opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity z-50 text-xs text-[var(--text-secondary)] font-normal">
                A strictly encrypted local store. Data stored here is decrypted
                in-browser and requires your PIN to view.
              </div>
            </div>
          </div>
          <p className="text-[var(--text-secondary)] text-sm">
      ...
      <div className="flex-1 overflow-y-auto pb-24 space-y-3">
        {entries.length === 0 ? (
          ...
          entries.map((entry) => {
            ...
            if (entry.category === "dsa")
              borderColor = "border-l-[var(--warning)]";
            else if (entry.category === "dev")
            ...
            return (
              <div
                key={entry.id}
                className={`p-4 bg-[var(--bg-surface)] border-y border-r border-l-4 ${borderColor} rounded-lg flex items-center justify-between`}
              >
                <div>
                  <span className="text-[10px] uppercase font-mono tracking-wider text-[var(--text-tertiary)] block mb-1">
                    {entry.category}
                  </span>
      ...
      {/* Slide-in Add Panel */}
      <div
        className={`fixed inset-y-0 right-0 w-full md:w-[400px] bg-[var(--bg-surface)] border-l border-[var(--border-hairline)] shadow-2xl transition-transform duration-300 ease-[cubic-bezier(0.16,1,0.3,1)] z-[100] ${isAdding ? "translate-x-0" : "translate-x-full"}`}
      >
          ...
          <form onSubmit={handleAddEntry} className="space-y-6 flex-1">
            <div className="space-y-1.5">
              ...
              <input
                ...
                placeholder="e.g. Supabase API Key"
            ...
            <div className="space-y-1.5">
              ...
              <input
                ...
                className="w-full bg-[var(--bg-base)] border border-[var(--border-hairline)] rounded-md px-3 py-2 text-sm font-mono outline-none focus:border-[var(--accent)]"
              />
            </div>
