...
export default function BacklogPage() {
  ...
  const fetchTasks = async () => {
    try {
      ...
      const today = allPending.filter((t: any) => t.scheduled_date === todayStr);
      ...
      const backlog = allPending.filter((t: any) => t.scheduled_date !== todayStr);
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    if (over && over.id === 'today-zone') {
      ...
      if (draggedTask) {
        ...
        setTodayTasks(prev => [...prev, { ...draggedTask, scheduled_date: new Date().toISOString().split('T')[0], is_pinned: true }]);

        try {
          const todayStr = new Date().toISOString().split('T')[0];
          await api.patch(`/micro-tasks/${draggedTask.id}`, { scheduled_date: todayStr, is_pinned: true });
        } catch (e) {
  ...
  const groupedBacklog = backlogTasks.reduce((acc, task) => {
  ...
  if (loading) return <div className="p-6 text-[var(--text-secondary)]">Loading Canvas...</div>;
        ...
        <div className="flex-1 grid md:grid-cols-2 gap-8 overflow-hidden">
          ...
          <div className="flex flex-col h-full border border-[var(--border-hairline)] rounded-xl bg-[var(--bg-surface)] overflow-hidden">
            <div className="p-4 border-b border-[var(--border-hairline)] bg-[var(--bg-surface-raised)]">
              <h2 className="font-medium text-[var(--text-primary)]">Backlog</h2>
            </div>
            <div className="flex-1 overflow-y-auto p-4 space-y-6">
              {Object.keys(groupedBacklog).length === 0 ? (
                <p className="text-center text-sm text-[var(--text-secondary)] mt-10">No backlogged tasks found.</p>
              ) : (
                Object.entries(groupedBacklog).map(([macroTitle, tasks]) => (
                  <div key={macroTitle} className="space-y-3">
                    <h3 className="text-xs font-mono uppercase tracking-wider text-[var(--text-tertiary)]">{macroTitle}</h3>
          ...
          <div className="flex flex-col h-full">
            <h2 className="font-medium text-[var(--text-primary)] mb-4 px-1">Today's Schedule</h2>
            <DroppableToday>
        ...
        <DragOverlay dropAnimation={dropAnimation}>
          {activeTask ? (
            <div className="p-3 bg-[var(--bg-surface-raised)] border border-[var(--accent)] rounded-lg flex items-center gap-3 shadow-none scale-105 cursor-grabbing">
              ...
              <span className="text-sm text-[var(--text-primary)]">{activeTask.title}</span>
            </div>
          ) : null}
        </DragOverlay>
      </DndContext>
