...
export default function NotesPage() {
  ...
  const colors = [
    { value: 'yellow', label: 'bg-yellow-500/20 text-yellow-500 border-yellow-500/30' },
    ...
    { value: 'green', label: 'bg-emerald-500/20 text-emerald-500 border-emerald-500/30' },
  ];
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    const data = await api.post('/notes', { content: newContent, type: newType, color: newColor });
  ...
  const toggleType = async (note: any) => {
    ...
    setNotes(prev => prev.map(n => n.id === note.id ? { ...n, type: newT } : n));
    await api.patch(`/notes/${note.id}`, { ...note, type: newT });
  };

  if (loading) return <div className="p-6 text-[var(--text-secondary)]">Loading notes...</div>;
    ...
    <div className="p-6 md:p-8 max-w-4xl mx-auto pb-32">
      ...
      <div className="bg-[var(--bg-surface)] border border-[var(--border-hairline)] rounded-xl p-4 mb-8">
        <form onSubmit={handleAdd} className="space-y-4">
          <textarea 
            required placeholder="Jot something down..."
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            ...
            <button type="submit" className="bg-[var(--text-primary)] text-[var(--bg-base)] px-4 py-1.5 rounded text-sm font-medium hover:opacity-90 flex items-center justify-center gap-2">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {notes.length === 0 ? (
          <p className="text-sm text-[var(--text-secondary)] col-span-2">No notes yet.</p>
        ) : (
          notes.map(note => {
            const colorClass = colors.find(c => c.value === note.color)?.label || colors[0].label;
            
            return (
              <div key={note.id} className={`p-4 border rounded-xl flex flex-col justify-between gap-4 ${colorClass}`}>
                {editingId === note.id ? (
                  <textarea 
                    autoFocus
                ...
                <div className="flex items-center justify-between border-t border-current/10 pt-3 mt-2">
                  <div className="flex items-center gap-2">
                    ...
                    >
                      {note.type === 'dashboard' ? '📌 Dashboard' : '📝 General'}
                    </button>
                  </div>
                  
                  <div className="flex items-center gap-2">
                    {editingId === note.id ? (
                      <button onClick={() => saveEdit(note.id, note)} className="p-1 hover:bg-black/10 rounded transition-colors">
                        <Check className="w-4 h-4" />
                      </button>
                    ) : (
                      <button onClick={() => startEditing(note)} className="p-1 hover:bg-black/10 rounded transition-colors">
                        <Edit2 className="w-4 h-4" />
                      </button>
                    )}
                    <button onClick={() => handleDelete(note.id)} className="p-1 hover:bg-black/10 rounded transition-colors">
