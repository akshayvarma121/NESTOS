...
export default function CalendarPage() {
  ...
  const handleAddEvent = async (e: React.FormEvent) => {
    ...
    try {
      const data = await api.post('/calendar/events', { title: newEventTitle, date: selectedDate });
    ...
    <div className="p-6 md:p-8 max-w-5xl mx-auto space-y-8 flex flex-col pb-32">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-4 bg-[var(--bg-surface-raised)] border border-[var(--border-hairline)] rounded-lg p-1">
          <button onClick={prevMonth} className="p-1.5 hover:bg-[var(--bg-base)] rounded transition-colors"><ChevronLeft className="w-4 h-4"/></button>
          <span className="text-sm font-medium min-w-[100px] text-center">{monthName}</span>
          <button onClick={nextMonth} className="p-1.5 hover:bg-[var(--bg-base)] rounded transition-colors"><ChevronRight className="w-4 h-4"/></button>
        </div>
      </div>
      ...
      <div className="bg-[var(--bg-surface)] border border-[var(--border-hairline)] rounded-xl overflow-hidden flex flex-col shadow-sm">
        ...
        <div className="grid grid-cols-7 border-b border-[var(--border-hairline)] bg-[var(--bg-surface-raised)]">
          {['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map(day => (
            <div key={day} className="p-2 text-center text-xs font-medium text-[var(--text-secondary)]">
              {day}
            </div>
          ))}
        </div>
        ...
        >
          {loading ? (
            ...
            if (closeout) {
               if (closeout.total_completed >= 5) bgClass = "bg-[#10b981]/20 hover:bg-[#10b981]/30";
               else if (closeout.total_completed > 0) bgClass = "bg-[#10b981]/10 hover:bg-[#10b981]/20";
            }
            ...
            return (
              ...
              >
                ...
                <div className="mt-1 space-y-1 overflow-hidden h-[40px] lg:h-[60px]">
                  {dayEvents.map(e => (
                    <div key={e.id} className="text-[10px] leading-tight bg-[var(--accent)]/10 text-[var(--accent)] border border-[var(--accent)]/20 rounded px-1 py-0.5 truncate">
                      {e.title}
                    </div>
                  ))}
                </div>
      ...
      <div className={`fixed inset-y-0 right-0 w-full md:w-[400px] bg-[var(--bg-surface)] border-l border-[var(--border-hairline)] shadow-2xl transition-transform duration-300 ease-[cubic-bezier(0.16,1,0.3,1)] z-[100] ${selectedDate ? 'translate-x-0' : 'translate-x-full'}`}>
        <div className="h-full flex flex-col p-6">
          <div className="flex justify-between items-center mb-6">
            ...
            <button onClick={() => { setSelectedDate(null); setIsAddingEvent(false); }} className="p-2 -mr-2 text-[var(--text-secondary)] hover:text-[var(--text-primary)]">
              <X className="w-5 h-5"/>
            </button>
          </div>
          ...
          <div className="flex-1 overflow-y-auto space-y-8 pr-2">
            ...
            <section className="space-y-3">
              ...
              {isAddingEvent && (
                <form onSubmit={handleAddEvent} className="flex gap-2 mb-4">
                  ...
                  />
                  <button type="submit" className="bg-[var(--accent)] text-white px-3 rounded-md text-sm font-medium">Add</button>
                </form>
              )}

              {selectedEvents.length === 0 && !isAddingEvent ? (
                <p className="text-xs text-[var(--text-secondary)]">No events scheduled.</p>
              ) : (
                <div className="space-y-2">
                  {selectedEvents.map(e => (
                    <div key={e.id} className="p-2 bg-[var(--accent)]/5 border border-[var(--accent)]/20 rounded-md text-sm text-[var(--text-primary)]">
                      {e.title}
                    </div>
                  ))}
                </div>
            ...
            {/* Analytics Section */}
            <section className="space-y-3">
              <h3 className="text-sm font-medium border-b border-[var(--border-hairline)] pb-2">Day Summary</h3>
              {selectedCloseout ? (
                <div className="grid grid-cols-2 gap-3">
                  <div className="p-3 bg-[var(--bg-surface-raised)] border border-[var(--border-hairline)] rounded-lg text-center">
                    <p className="text-xs text-[var(--text-secondary)] uppercase tracking-wider mb-1">Completed</p>
                    <p className="text-xl font-semibold text-[#10b981]">{selectedCloseout.total_completed}</p>
                  </div>
                  <div className="p-3 bg-[var(--bg-surface-raised)] border border-[var(--border-hairline)] rounded-lg text-center">
                    <p className="text-xs text-[var(--text-secondary)] uppercase tracking-wider mb-1">Scheduled</p>
                    <p className="text-xl font-semibold">{selectedCloseout.total_scheduled}</p>
                  </div>
                </div>
              ) : (
                <p className="text-xs text-[var(--text-secondary)]">No close-out data for this day.</p>
              )}
            </section>
            ...
            {selectedTasks.length > 0 && (
              <section className="space-y-3">
                <h3 className="text-sm font-medium border-b border-[var(--border-hairline)] pb-2">Completed Tasks</h3>
                <div className="space-y-1.5">
                  {selectedTasks.map(t => (
                    <div key={t.id} className="flex items-center gap-2 p-2 bg-[var(--bg-surface-raised)] border border-[var(--border-hairline)] rounded-md">
                      <div className="w-1.5 h-1.5 rounded-full bg-[#10b981]"></div>
                      <span className="text-sm text-[var(--text-secondary)] line-through">{t.title}</span>
                    </div>
                  ))}
                </div>
              </section>
      ...
      {selectedDate && <div className="fixed inset-0 bg-black/40 z-[90] backdrop-blur-sm lg:hidden" onClick={() => setSelectedDate(null)} />}
    </div>
  );
}
