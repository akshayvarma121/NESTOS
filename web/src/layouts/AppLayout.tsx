...
import { Plus, Bell, X, WifiOff } from 'lucide-react';
  ...
  useEffect(() => {
    ...
    if ('Notification' in window && 'serviceWorker' in navigator) {
      if (Notification.permission === 'default' && !localStorage.getItem('push_dismissed')) {
        // Show after a small delay to feel contextual, or usually we'd wait for a specific user action.
        // For simplicity, we delay it 3 seconds on load, but in a real app we might tie it to "first goal creation".
    ...
    <div className="flex h-[100dvh] w-full bg-[var(--bg-base)] overflow-hidden">
      ...
      {isOffline && (
        <div className="fixed top-0 left-0 right-0 bg-[var(--bg-surface-raised)] border-b border-[var(--border-hairline)] px-4 py-1.5 flex items-center justify-center gap-2 z-[60]">
          ...
          <span className="text-xs text-[var(--text-secondary)] font-medium">Offline — showing last synced data</span>
        </div>
      )}
        ...
        {showPushBanner && (
          <div className="mx-6 mb-6 p-4 bg-[var(--bg-surface)] border border-[var(--border-hairline)] rounded-xl flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 animate-in fade-in slide-in-from-top-2">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm font-medium">Get a morning briefing on your phone?</p>
                <p className="text-xs text-[var(--text-secondary)]">Plus alerts for upcoming application deadlines.</p>
              </div>
            </div>
            <div className="flex items-center gap-3 w-full sm:w-auto">
              ...
              <button onClick={dismissPush} className="p-2 text-[var(--text-secondary)] hover:text-[var(--text-primary)]">
                <X className="w-4 h-4" />
              </button>
            </div>
          </div>
