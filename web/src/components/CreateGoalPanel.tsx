...
export default function CreateGoalPanel({ isOpen, onClose, onSubmit }: Props) {
  ...
  const updateSlice = (id: string, field: string, value: string) => {
    setSlices(prev => prev.map(s => s.id === id ? { ...s, [field]: value } : s));
  };
      ...
      <div className="fixed inset-y-0 right-0 w-full md:w-[450px] bg-[var(--bg-surface-raised)] border-l border-[var(--border-hairline)] z-50 flex flex-col shadow-2xl animate-in slide-in-from-right duration-200">
        ...
        <div className="flex items-center justify-between p-4 md:p-6 border-b border-[var(--border-hairline)]">
          <div className="flex items-center gap-3">
            {step === 2 && (
              <button onClick={() => setStep(1)} className="p-1 hover:bg-[var(--border-hairline)] rounded-md text-[var(--text-secondary)]">
                <ArrowLeft className="w-5 h-5" />
              </button>
            )}
            <h2 className="text-lg font-semibold">{step === 1 ? 'New Macro Goal' : 'Edit Goal Slices'}</h2>
          </div>
        ...
        {step === 1 ? (
          <form onSubmit={handleNext} className="p-4 md:p-6 flex-1 overflow-y-auto space-y-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1.5">
                <label className="text-sm font-medium text-[var(--text-secondary)]">Total Units (Slices)</label>
                <input 
          ...
          <div className="p-4 md:p-6 flex-1 overflow-y-auto space-y-4 flex flex-col">
            <p className="text-xs text-[var(--text-secondary)] mb-2">
              Your goal has been sliced into {slices.length} micro-tasks. You can rename them or pre-assign them to specific dates. Any task without a date will go to the Canvas Backlog.
            </p>
            
            <div className="flex-1 overflow-y-auto space-y-3 bg-[var(--bg-base)] border border-[var(--border-hairline)] rounded-xl p-3">
              {slices.map((slice, i) => (
                <div key={slice.id} className="flex flex-col gap-1 pb-3 border-b border-[var(--border-hairline)] last:border-0 last:pb-0">
                  <div className="flex items-center gap-2">
                    <span className="text-xs font-mono text-[var(--text-tertiary)] w-6 text-right">{i+1}.</span>
                    <input 
                      value={slice.title}
                      onChange={e => updateSlice(slice.id, 'title', e.target.value)}
                  ...
                  <div className="flex items-center gap-2 pl-8">
                    <input 
                      ...
                      onChange={e => updateSlice(slice.id, 'scheduled_date', e.target.value)}
                    ...
                    />
                    <span className="text-[10px] text-[var(--text-tertiary)]">Optional Date</span>
                  </div>
                </div>
              ))}
            </div>
