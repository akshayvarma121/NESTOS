      ...
      <div className="fixed inset-y-0 right-0 w-full md:w-[450px] bg-[var(--bg-surface-raised)] border-l border-[var(--border-hairline)] z-50 flex flex-col shadow-2xl animate-in slide-in-from-right duration-200">
        ...
        <div className="flex items-center justify-between p-4 md:p-6 border-b border-[var(--border-hairline)]">
          <div>
            ...
            <p className="text-xs text-[var(--text-secondary)] mt-1">Configure your daily repeating routines</p>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-8">
          ...
          {/* Create Form */}
          <form onSubmit={handleCreate} className="space-y-4 p-4 border border-[var(--border-hairline)] rounded-lg bg-[var(--bg-surface)]">
            <h3 className="text-sm font-medium">Add Routine Block</h3>
            
            <div className="grid grid-cols-2 gap-3">
              <input 
                required placeholder="Title (e.g. Wake up)"
          ...
          <div className="space-y-3">
            <h3 className="text-sm font-medium border-b border-[var(--border-hairline)] pb-2">Active Routines</h3>
            {loading ? (
              ...
              routines.map(routine => (
                <div key={routine.id} className="p-3 border border-[var(--border-hairline)] rounded-lg flex items-center gap-4 bg-[var(--bg-surface)] flex-wrap">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm text-[var(--text-primary)] truncate">{routine.title}</p>
