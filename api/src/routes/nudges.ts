...
router.post("/", async (req: AuthRequest, res) => {
  ...
  const { data: connections } = await supabase
    ...
    .select("*")
    .or(
      `and(user_id.eq.${req.user!.id},partner_id.eq.${partner_id}),and(user_id.eq.${partner_id},partner_id.eq.${req.user!.id})`,
    );
...
router.get("/history", async (req: AuthRequest, res) => {
  const { data, error } = await supabase
    .from("pos_nudges")
    .select(
      "*, sender:pos_user_profiles!pos_nudges_sender_id_fkey(username), receiver:pos_user_profiles!pos_nudges_receiver_id_fkey(username)",
    )
