...
const vaultSessions = new Map<
  string,
  { aesKey: Buffer; expiresAt: number; spaceIds: string[] }
>();
...
router.post("/setup", async (req: AuthRequest, res) => {
  ...
  if (existing)
    return res.status(400).json({ error: "Shared Vault already configured" });
...
router.post("/unlock", async (req: AuthRequest, res) => {
  ...
  const { data: security, error } = await getSpaceVaultConfig(
    req.sharedSpaceIds!,
  );
  if (error || !security)
...
router.post("/entries", async (req: AuthRequest, res) => {
  ...
  if (!aesKey)
    return res.status(401).json({ error: "Vault locked or session expired" });
...
router.delete("/reset", async (req: AuthRequest, res) => {
  ...
  for (const [token, session] of vaultSessions.entries()) {
    const hasOverlap = session.spaceIds.some((id) =>
      req.sharedSpaceIds!.includes(id),
    );
    if (hasOverlap) {
      vaultSessions.delete(token);
    }
